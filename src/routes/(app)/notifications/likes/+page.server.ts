...
export const load = async(request) =>{
    ...
    .where(and(eq(notificationsTable.targetUser, session.userId), or(eq(notificationsTable.entityType, "comment_like"), eq(notificationsTable.entityType, "post_like"))))
...
async function fetchNotifications(notifications: NotificationType[]){
        ...
        if (notif.notification.entityType === "post_like"){
